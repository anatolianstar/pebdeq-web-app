...
const ProductDetail = () => {
  ...
  const getCurrentImages = () => {
    ...
    if (product.video_url) {
      media.unshift({ type: 'video', url: product.video_url });
    }
    
    // Convert string URLs to objects for consistency
            ...
            <div className="main-image" onClick={() => images.length > 0 && siteSettings.product_detail_show_image_lightbox && openLightbox(selectedImage)}>
              {images.length > 0 ? (
                images[selectedImage]?.type === 'video' ? (
                  ...
                  >
                    <source src={images[selectedImage].url} type="video/mp4" />
                    <source src={images[selectedImage].url} type="video/webm" />
                    <source src={images[selectedImage].url} type="video/avi" />
              ...
              <div className="image-thumbnails">
                {images.map((media, index) => (
                  <div
                    ...
                    title={media.type === 'video' ? "Click to play video" : (siteSettings.product_detail_show_image_lightbox ? "Double click for large image" : "")}
                  >
                    {media.type === 'video' ? (
          ...
          <div className="lightbox-overlay" onClick={closeLightbox}>
            <div className="lightbox-content" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="lightbox-image-container">
                {images[lightboxImageIndex]?.type === 'video' ? (
                  <video 
                  ...
                  >
                    <source src={images[lightboxImageIndex].url} type="video/mp4" />
                    <source src={images[lightboxImageIndex].url} type="video/webm" />
                    <source src={images[lightboxImageIndex].url} type="video/avi" />
                    Your browser does not support the video tag.
                  </video>
                ) : (
                  <img 
                ...
                <div className="lightbox-info">
                  <h3>{product.name}</h3>
                  <p>{lightboxImageIndex + 1} / {images.length} {images[lightboxImageIndex]?.type === 'video' ? '(Video)' : '(Resim)'}</p>
                </div>
              </div>
              ...
              {images.length > 1 && (
                <div className="lightbox-thumbnails">
                  {images.map((media, index) => (
                    ...
                    >
                      {media.type === 'video' ? (
                        <div className="lightbox-video-thumbnail">
                          <video 
                            src={media.url}
                            style={{ 
                              width: '60px', 
