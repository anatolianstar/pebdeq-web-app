...
def create_rollback_point():
    ...
    try:
        ...
        backup_base = "Backups" if (project_root / "pb_test_suite" / "Backups").exists() else "backups"
        rollback_dir = project_root / "pb_test_suite" / backup_base / "rollback_points" / f"rollback_{timestamp}"
...
def create_approved_backup():
    ...
    try:
        ...
        # Detect correct backup directory case (Backups vs backups)
        backup_base = "Backups" if (project_root / "pb_test_suite" / "Backups").exists() else "backups"
        rollback_dir = project_root / "pb_test_suite" / backup_base / "rollback_points" / folder_name
        rollback_dir.mkdir(parents=True, exist_ok=True)
        ...
        if manual_backup and selected_files_env:
            ...
            try:
                ...
                print(f"[DEBUG] Raw SELECTED_FILES env: '{selected_files_env}'")
                ...
                if all(path.isdigit() for path in selected_file_paths):
                    ...
                    all_files_with_ids = []
                    for file_type, files in tester.all_files.items():
                        print(f"[DEBUG] Adding {len(files)} files from {file_type} category")
                        all_files_with_ids.extend(files)
                    
                    print(f"[DEBUG] Total available files: {len(all_files_with_ids)}")
                    print(f"[DEBUG] First 3 available files:")
                    for i, file_info in enumerate(all_files_with_ids[:3]):
                    ...
                    print(f"[DEBUG] Files to backup: {[f['relative_path'] for f in files_to_backup[:5]]}")
            ...
            # Individual backup mode: backup only the tested file
            if single_file_mode and backup_file_name:
                print(f"[INFO] Individual backup mode: Creating backup for {backup_file_name}")
                
                # Find the specific file that was tested
                target_file_found = False
                for file_type, files in tester.all_files.items():
                    for file_info in files:
                        file_path_str = str(file_info['relative_path'])
                        # Check if this is the tested file
                        if (backup_file_name in file_path_str or 
                            file_path_str.endswith(backup_file_name) or
                            file_path_str.split('/')[-1] == backup_file_name):
                            files_to_backup.append(file_info)
                            target_file_found = True
                            print(f"[SINGLE-FILE] Found and added: {file_path_str}")
                            break
                    if target_file_found:
                        break
                
                if not target_file_found:
                    print(f"[WARNING] Could not find tested file: {backup_file_name}")
                    print(f"[FALLBACK] Adding all files for safety")
                    # Fallback to all files if target file not found
                    for file_type, files in tester.all_files.items():
                        files_to_backup.extend(files)
                else:
                    print(f"[SUCCESS] Will backup only the tested file: {len(files_to_backup)} file(s)")
            else:
                # No single file specified, backup all files
                print(f"[FULL-BACKUP] No specific file specified, backing up all files")
                for file_type, files in tester.all_files.items():
                    files_to_backup.extend(files)
        ...
        print(f"\n[BACKUP-START] Starting backup process...")
        print(f"[BACKUP-START] Total files to backup: {len(files_to_backup)}")
        print(f"[BACKUP-START] Backup directory: {rollback_dir}")
        ...
        print(f"\n[BACKUP-COMPLETE] Backup loop finished!")
        print(f"[BACKUP-COMPLETE] Successfully backed up: {len(backed_up_files)} files")
        print(f"[BACKUP-COMPLETE] Failed/skipped: {len(files_to_backup) - len(backed_up_files)} files")
...
def list_rollback_points():
    ...
    try:
        # Detect correct backup directory case (Backups vs backups)
        backup_base = "Backups" if (project_root / "pb_test_suite" / "Backups").exists() else "backups"
        rollback_dir = project_root / "pb_test_suite" / backup_base / "rollback_points"
