...
// Dosyaları ve import yollarını define et
const files = [
...
function addImportToFile(filePath, importPath) {
  try {
    ...
    // Zaten import var mı kontrol et
    if (content.includes("createApiUrl")) {
      console.log(`⚡ ${filePath} - import already exists`);
      return;
    }
    
    // createApiUrl kullanılıyor mu kontrol et
    if (!content.includes("createApiUrl(")) {
      console.log(`⚡ ${filePath} - createApiUrl not used, skipping`);
      return;
    }
    ...
    if (insertIndex === -1) {
      // Import bulunamadı, dosyanın başına ekle
      insertIndex = 0;
    }
...
// Ana işlev
function fixImportsMain() {
  console.log('🔧 Fixing missing createApiUrl imports...');
